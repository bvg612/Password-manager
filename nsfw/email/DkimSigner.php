...
class DkimSigner{
  ...
  // AGENT PROPERTIES //
  //////////////////////
  private $appName = "PHP mailDomainSigner";
  private $appVer = "0.2-20110415";
  private $appUrl	= "http://code.google.com/p/php-mail-domain-signer/";

  /**
   * Constructor
   *
   * @param string $private_key Raw Private Key to Sign the mail
   * @param string $domain      The domain name of the signing domain
   * @param string $selector    The selector used to form the query for the public key
   *
   * @author Ahmad Amarullah
   */
  ...
  public function getDKIM($h,$_h,$body) {
    ...
    $_dkim = "DKIM-Signature: ".
                ...
                "s={$this->selector}; ".         // The selector subdividing the namespace for the "d=" (domain) tag
                "d={$this->domain}; ".         // The domain of the signing entity
  ...
  public function sign(
  	...
  	if (!$suggested_h) $suggested_h = "from:to:subject"; // Default Suggested Signed Header Fields
		...
		$_nh = array("x-domain-signer"=>"X-Domain-Signer: {$this->appName} {$this->appVer} <$this->appUrl>");
